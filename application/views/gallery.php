          ...
          foreach ($gallery as $image) { ?>
    			<div class="col-xs-4 col-md-4">                  
    				<div class="thumbnail">					
    					...
    					?>
    					<a href="<?=base_url();?>uploads/gallery/<?=$image->file_name?>" class="fancybox" title="<?//=$image->name;?>" rel="gallery">
    						<img src="<?=base_url();?>uploads/gallery/<?=$thumb?>" alt="<?//=$image->name;?>" />
    					</a>
    				</div>
    				<div class="bottomleft"><?=$this->user_model->get_user($image->part_id)->name;?>
              <?php if ($user_id == $image->part_id) { ?>
              <i class="glyphicon glyphicon-heart pull-right">
                <span class="hit"><?=$this->gallery_model->check_score($image->part_id, $image->id);?></span>
              </i>              
    				  <?php } else { ?>
